"""Usage: pj_run_code.py [-h][--cluster=<arg>][--num_sigma=<arg>][--red_clump=<arg>][--run_number=<arg>][--location=<arg>][--elem=<arg>]
...
def run_everything(cluster, num_sigma, red_clump, run_number, location, elem):
    ...
    apogee_cluster_data, spectra, spectra_errs, T, bitmask = pj.get_spectra(cluster, red_clump, location)
    num_elem = 15
    ...
    real_weights = [] 
    for i in range(len(sigma_vals)):
        real_dat = pj.fit_func(elem, cluster, final_real_spectra[i], final_real_spectra_err[i], T, dat_type = 'data', run_number = run_number, location = location, sigma_val=None)
