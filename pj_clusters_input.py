"""Usage: pj_clusters_input.py [-h][--cluster=<arg>][--red_clump=<arg>][--element=<arg>][--type=<arg>][--location=<arg>]
...
def photometric_Teff(apogee_cluster_data): #Probably going to take this out and use spectroscopic Teffs
...
def get_spectra(name, red_clump, location):
	...
	#If the file does not exist
		...
		if location == 'personal':
			apogee_cluster_data = np.load('/Users/chloecheng/Personal/published_clusters.npy') 
		elif location == 'server':
			apogee_cluster_data = np.load('/geir_data/scr/ccheng/AST425/Personal/published_clusters.npy') 
