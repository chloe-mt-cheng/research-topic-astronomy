"""Usage: pj_ABC.py [-h][--cluster=<arg>][--red_clump=<arg>][--sigma=<arg>]
...
def get_cluster_data(cluster, red_clump):
    ...
    apogee_cluster_data, spectra, spectra_errs, T, bitmask = pj.get_spectra(cluster, red_clump)
    num_elem = 15
...
def psm_data(num_elem, num_stars, apogee_cluster_data, sigma, T, cluster, spectra, spectra_errs, run_number):
    ...
    for i in range(len(elem_dict['element'])):
        fake_dat = pj.fit_func(elem_dict['element'][i], cluster, final_fake_spec, final_real_spectra_err, T, dat_type = 'sim', run_number = run_number, sigma_val=sigma)
        if fake_dat is None:
...
def real_data(num_elem, cluster, spectra, spectra_errs, T, run_number):
    ...
    real_used_elems = []
    for i in range(len(elem_dict['element'])):
        real_dat = pj.fit_func(elem_dict['element'][i], cluster, spectra, spectra_errs, T, dat_type = 'data', run_number = run_number, sigma_val=None)
