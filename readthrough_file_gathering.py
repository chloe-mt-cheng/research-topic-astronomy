...
import os ###Getting pathnames, deleting files, etc.
import h5py ###Writing files
import numpy as np ###Numpy
import time ###For file labelling
import glob ###Check if a file exists or not
import shutil ###Delete the whole directory
...
def gather_files(name, location, elem):
    ...
    cluster = str(name) ###Cluster name
    timestr = time.strftime("%Y%m%d_%H%M%S") ###Date and time 
    elem_dict = {'element': ['C', 'N', 'O', 'NA', 'MG', 'AL', 'SI', 'S', 'K', 'CA', 'TI', 'V', 'MN', 'FE', 'NI']} ###Dictionary of all elements in APOGEE
    ...
    files = os.listdir(path) ###Get all file names in run_files
    res_data_files = [] ###Empty list for the fit information from the data
    res_sim_files = [] ###Empty list for the fit information from the simulations
    dcov_files = [] ###Empty list for the delta covariance statistics
    ks_files = [] ###Empty list for the KS distance statistics
    algorithm_files = [] ###Empty list for the files that save Dcov, KS, sigma
    for file in files: ###Iterate through the files and add the corresponding ones to the corresponding lists 
        if 'res_data' in file:
    ...
    j = 0    ###This was only relevant when I was running all of the elements at once
    res_data_0_files = []
